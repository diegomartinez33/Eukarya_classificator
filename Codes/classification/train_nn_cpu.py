...
def test_final(test_loader, epoch, best_model_file):
    defineModel(load_net=True, model_file=best_model_file)
    model.eval()
    correct = 0
    with torch.no_grad():
        for batch_idx, (data, target) in enumerate(test_loader):
...
def main(train_data, train_labels, test_data, test_labels, num_fold=0, Net_type='mnist_net', Ei=1):
    ...
    try:
        ...
        train_results = test_final(loaders[1], epoch, best_model_file)
